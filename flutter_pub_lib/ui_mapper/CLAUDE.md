...
詳細な設計判断の経緯は `docs/screen_designer_design_v4.md` を参照してください（このリポジトリに配置する場合）。実装で迷ったら、まずそちらの「9. 確定した設計判断」と「11. 実装検証で発見した不具合と修正」を確認してください。

## 現在の状態（引き継ぎ時点）

- 4パッケージ（model/grid/runtime/editor）の設計は一通り確定。`NodeKind`（widget/container/spacer）の明示フィールド化まで完了し、テスト実装で発見した不具合も概ね修正済み。
...
- **JSONマイグレーション未実装**：`nodeType`追加により、旧形式のJSON（`nodeType`キー無し）は読み込めなくなった。既存の保存済みJSONがあるかどうか、マイグレーションが必要かどうかをユーザーに確認すること。
...
- `DesignNode` は `NodeKind`（`widget` / `container` / `spacer`）を明示フィールドとして持つ。**旧方針（name/childrenの組み合わせから推測し、新フィールドを追加しない）は撤回済み。** 理由：新規作成直後の「子が0件のコンテナ」と「意図的な空きセル」が実データ上区別できず、キャンバス上で意図せず0サイズ扱いになる不具合が発生したため（設計書11.8）。
  - `nodeType == widget` → `name`必須。`children`を持つことも許可する（Widgetの下にさらにデザインを続けられる）
  - `nodeType == container` → `name`不可。子を並べる
  - `nodeType == spacer` → `name`不可、`children`不可。何も描画しない空きセル
  - これらの整合性は`DesignNode`のコンストラクタ本体で検証し、矛盾する組み合わせは`ArgumentError`で即座に弾く。
  - `nodeType`は**作成後に変更不可**。エディタの`NodeEditDialog`は既存ノード編集時、種別選択UI（`SegmentedButton`）を無効化する。
- `NodeBreakpointStyle` の未指定区分（`styles`マップにキーがない）は、より小さい区分（compact側）を継承する。**`compact` は必須**（省略された場合は `ArgumentError`）。継承チェーンを解決するロジックは**必ず「該当区分から0まで遡る」実装にすること**（「1つ手前だけ見る」実装は2段階遡るケースで壊れる。過去に実際に発生した不具合）。
...
- **`DragTarget`は「兄弟として挿入」用と「子として追加」用を別々に用意すること。** コンテナの中身（背景・空欄部分）専用の`DragTarget`（`targetParent: そのコンテナ自身`）が無いと、ドラッグ&ドロップでコンテナに子を追加する操作ができない。既存の子への上書きドロップ（兄弟挿入）とコンテナ自体への追加ドロップは別物として両方実装する。
- **コンテナへの子ドロップを実装したら、必ず循環参照防止（`containsDescendant`）とセットで入れること。** あるノードを自分自身の子孫の中へドロップできてしまうと、ツリーが循環し描画が無限ループする。

## ドキュメント更新について

このリポジトリの設計判断が変わった場合は、`docs/screen_designer_design_v4.md`（設計書）の「9. 確定した設計判断」「10. 未着手・今後の検討事項」「11. 実装検証で発見した不具合と修正」を合わせて更新してください。設計書とこの `CLAUDE.md` の内容に矛盾が生じないようにすること。
