# ui_mapper（screen_designer）ライブラリ設計書 v3

> v2からの変更点: 実装検証（VSCode移行前の最終確認）で発見した3件の不具合と修正を反映。詳細は「11. 実装検証で発見した不具合と修正」を参照。
...
`name`と`children`の組み合わせで3つの意味を持つ。

- `name != null` → アプリが実行時に解決する実体Widget
- `name == null` かつ `children` あり → 子を並べるコンテナ
- `name == null` かつ `children` が空 → 何も描画しない空きセル（オフセット）

未指定区分は手前（compact側）を継承。**compactは必須**。
...
class DesignNode {
  ...
  DesignNode({ // v3: constを削除し、コンストラクタ本体でcompact必須を検証する
    required this.id,
    this.name,
    this.children = const [],
  ...
  factory DesignNode.fromJson(Map<String, dynamic> json) {
    ...
    if (!styles.containsKey(Breakpoint.compact)) {
      throw FormatException('DesignNode(id: ${json['id']}) に compact のstyleが必要です。');
    }
    return DesignNode(
...
class DesignEditorController extends ChangeNotifier {
  ...
  /// v3: stylesにcompactが無くてもデフォルト値で補う（DesignNodeコンストラクタの
  /// compact必須チェックに引っかからないようにする防御の2段目）。
  DesignNode addNode({DesignNode? parent, String? name, Map<Breakpoint, NodeBreakpointStyle>? styles}) {
    ...
    final newNode = DesignNode(id: _generateId(), name: name, styles: effectiveStyles);
...
- `name`が`null`かつ`children`が空のノードは空きセル（オフセット）。モデルへの新規フィールド追加は不要。
...
- スキーマバージョンが上がった場合のマイグレーション処理（`DesignJsonCodec._assertSupportedVersion`内に実装予定）
- ドラッグ操作の開始トリガー（現在は`LongPressDraggable`。専用ドラッグハンドルへの変更も検討可）
- コンテナが空きセル状態（`isLeaf`）から子を持つ状態に変わった際の、キャンバス上での見た目の遷移（サイズが急に変わることへの配慮）
...
**症状1**: 名前未設定のWidgetノードや、子がまだ0件のコンテナノードが、キャンバス上でほぼ見えない大きさ（1px程度）で表示される。
**根本原因（2段階）**:
...
## フォロー
11.4:この候補について、Widgetの場合に名前が空のときは保存ボタンをDisableにする。
11.7:症状1は改善されたが、正方形に表示され、ゴミ箱アイコンがはみ出して右端に表示される。
追加:ドラッグドロップが有効にならない。そのため、行の中にWidgetをドロップするテストができない。