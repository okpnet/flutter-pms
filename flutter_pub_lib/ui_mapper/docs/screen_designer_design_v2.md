# ui_mapper（screen_designer）ライブラリ設計書 v2
...
class DesignEditorController extends ChangeNotifier {
  ...
  DesignNode addNode({DesignNode? parent, String? name, Map<Breakpoint, NodeBreakpointStyle>? styles}) {
    ...
    final newNode = DesignNode(
      id: _generateId(),
      name: name,
      styles: styles ?? const {Breakpoint.compact: NodeBreakpointStyle(visible: true, width: 1)},
    );
  ...
  void replaceNodeStyles(DesignNode node, Map<Breakpoint, NodeBreakpointStyle> styles) {
    mutateTree((rootNodes) {
      ...
      target?.styles..clear()..addAll(styles);
    });
  }
...
構成:
- `DesignCanvas`: rootNodesから最初の`_CanvasRow`群を構築
- `_CanvasRow(nodes, controller, parent)`: 1行分。`DragTarget`+`LongPressDraggable`で移動を扱う
- `_CanvasCell(node, controller)`: 枠線・選択状態・編集/削除アイコン・リサイズハンドル
- `_NodeContent(node, controller)`: Widget実体 / 空きセル / コンテナ（再帰）の3分岐
