...
### 画面仕様
1.  ~~source/views.md~~schema.jsonをもとにview.yamlによる//0033
1. kindによって異なる//0033
...
2026/09/24_0033:大幅な方針変更。[画面仕様](#画面仕様)を変更した。既存のGraphQL、生成物を一度削除し、GraphQLの再生成のテストを行い、問題が見つからなかった場合はCLAUD.mdの見え消し部分を削除する。この0033は1度のみ実行。