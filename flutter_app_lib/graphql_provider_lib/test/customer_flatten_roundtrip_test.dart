// 要件0018・0025: customer画面のGraphQLから build_runner で生成されたモデルの
...
import 'package:gqlprvlib/postgraphile/customer/customer_edit.graphql.dart';
import 'package:gqlprvlib/postgraphile/customer/customer_read.graphql.dart';
...
void main() {
  test('要件0025: Query\$CustomerRead <-> Map 往復変換が元と一致する', () {
    final model = Query$CustomerRead.fromJson(_buildRawReadJson());
    ...
    final restoredNode = Query$CustomerRead$allMstrStakeholders$nodes.fromJson(restoredMap);

    expect(restoredMap, equals(nodeMap));
    expect(restoredNode, equals(node));

    // ignore: avoid_print
    print('CUSTOMER_READ_FLAT_MAP_JSON=${jsonEncode(flatMap)}');
  });

  test('要件0025: Mutation\$CustomerEdit <-> Map 往復変換が元と一致する', () {
    final model = Mutation$CustomerEdit.fromJson(_buildRawEditJson());
    ...
    // ignore: avoid_print
    print('CUSTOMER_EDIT_FLAT_MAP_JSON=${jsonEncode(flatMap)}');
  });
}
