// 要件0018・0025: customer_contact画面のGraphQLから build_runner で生成されたモデルの
...
import 'package:gqlprvlib/postgraphile/customer_contact/customer_contact_edit.graphql.dart';
import 'package:gqlprvlib/postgraphile/customer_contact/customer_contact_read.graphql.dart';
...
void main() {
  test('要件0025: Query\$CustomerContactRead <-> Map 往復変換が元と一致する', () {
    final model = Query$CustomerContactRead.fromJson(_buildRawReadJson());
    ...
    final restoredNode = Query$CustomerContactRead$allMstrStakeholderContacts$nodes.fromJson(
      restoredMap,
    );
    ...
    // ignore: avoid_print
    print('CUSTOMER_CONTACT_READ_FLAT_MAP_JSON=${jsonEncode(flatMap)}');
  });

  test('要件0025: Mutation\$CustomerContactEdit <-> Map 往復変換が元と一致する', () {
    final model = Mutation$CustomerContactEdit.fromJson(_buildRawEditJson());
    ...
    // ignore: avoid_print
    print('CUSTOMER_CONTACT_EDIT_FLAT_MAP_JSON=${jsonEncode(flatMap)}');
  });
}
